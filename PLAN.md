...
- Нужно выбрать конкретный SQLite driver для Go.
- Нужно выбрать подход к миграциям: встроенные SQL-файлы, goose, golang-migrate или собственный минимальный migrator.
- Нужно решить, какой LLM provider подключать первым.
- Нужно определить формат конфигурации: env vars, config file или смешанный вариант.
- Нужно выбрать HTML rendering подход: `html/template` в stdlib или лёгкий роутер/шаблонизатор.

На первом этапе эти решения лучше принимать консервативно: использовать минимальные зависимости и простую структуру, пока pipeline не заработает end-to-end.
